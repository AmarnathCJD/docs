---
title: "InlineQuery"
description: "Handle inline queries and build inline results"
---
    ...
    builder.Article(
        ...
        &telegram.ArticleOptions{
            ...
            ReplyMarkup: telegram.ReplyInlineMarkup(
                telegram.Row(
                    telegram.URL("Visit", "https://example.com"),
                ),
            ),
            LinkPreview: false,
        },
...
<Tabs>
  ...
  <Tab title="With Buttons">
    ...
    builder.Photo("photo.jpg", &telegram.ArticleOptions{
        ...
        ReplyMarkup: telegram.ReplyInlineMarkup(
            telegram.Row(
                telegram.URL("Open", "https://example.com"),
                telegram.Data("Like", "like"),
            ),
        ),
    })
    ```
...
client.On("inline", func(q *telegram.InlineQuery) error {
    ...
    for i, img := range images {
        builder.Photo(img.URL, &telegram.ArticleOptions{
            ...
            ReplyMarkup: telegram.ReplyInlineMarkup(
                telegram.Row(
                    telegram.URL("View Full", img.URL),
                ),
            ),
        })
    }
...
<CardGroup cols={2}>
  <Card title="Fast Response" icon="bolt">
    Answer queries quickly (< 1 second)
  </Card>
  <Card title="Relevant Results" icon="bullseye">
    Show relevant results based on query
  </Card>
  ...
  <Card title="Private Results" icon="lock">
    Use private flag for user-specific data
  </Card>
  <Card title="Unique IDs" icon="fingerprint">
    Use unique IDs for each result
  </Card>
</CardGroup>
