...
<Tip>
  **Cache File Protection**: Cache files are now automatically bound to the user ID of the logged-in bot or user account. This prevents the use of cache files created with one session by another session, eliminating issues caused by invalid access hashes.
</Tip>
